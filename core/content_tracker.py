...
class ContentTracker:
    def __init__(self, db_path="data/posts.db", db_client=None):
    ...
    def _get_recent_posts(self, days: int, allowed_statuses: Optional[List[str]] = None) -> List[Dict]:
        """Return recent posts from the active backing store."""
        cutoff = datetime.now() - timedelta(days=days)
        allowed_statuses = set(allowed_statuses or [])

        if self.db_client is not None:
            posts = self.db_client.get_posts(limit=1000)
            filtered_posts = []
            ...
            for post in posts:
                ...
                if created_at < cutoff:
                    continue
                if allowed_statuses and post.get("status") not in allowed_statuses:
                    continue
        ...
        if allowed_statuses:
            ...
            cursor.execute(
                f"""
                SELECT id, pillar, format, topic, text, created_at, status, voice_score, length, hashtags
                FROM posts
                WHERE created_at >= ?
                AND status IN ({placeholders})
                ORDER BY created_at
                """,
                (cutoff_date, *allowed_statuses)
            )
        else:
            cursor.execute(
                """
    ...
    def get_pillar_balance(self, days: int = 30) -> Dict:
        ...
        """
        posts = self._get_recent_posts(days=days, allowed_statuses=["published", "scheduled"])
        results = Counter(post.get("pillar") for post in posts if post.get("pillar"))
        ...
        current_dist = {pillar: count / total for pillar, count in results.items()}
    ...
    def get_dashboard(self) -> Dict:
        ...
        balance_items = list(pillar_balance["balance"].values())
        if balance_items:
            balance_score = sum(
                100 if data["status"] == "✓" else 50 if data["status"] == "⚠️" else 0
                for data in balance_items
            ) / len(balance_items)
        else:
            balance_score = 0
