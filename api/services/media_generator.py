...
class MediaGenerator:
    ...
    def generate_carousel_pdf(
        ...
        # Cap at 5 content slides so total PDF pages = 6 (cover + 5 slides)
        if len(slides) > 5:
            print(f"DEBUG: Capping slides from {len(slides)} to 5 (6 total pages including cover)")
            slides = slides[:5]
        ...
        image_prompts = {}
        no_text_instruction = "The image must contain ABSOLUTELY NO text, NO letters, NO words, NO numbers, NO typography, NO captions, NO labels, NO signs, NO watermarks, NO writing of any kind. Pure visual only."
        
        # Reasoning-first scene generation: feed full slide content so Gemini
        # identifies the specific claim/action/result before describing a scene.
        style_visual_tone = {
            'professional': 'cinematic, dramatic lighting, high contrast, moody and authoritative',
            'relaxed': 'warm natural light, candid, earthy tones, approachable',
            'corporate': 'sleek, polished, sharp executive feel, cool blue tones',
            'creative': 'bold colors, dynamic angles, vibrant and energetic',
            'minimal': 'clean, airy, high-key light, understated'
        }
        visual_tone = style_visual_tone.get(style, style_visual_tone['professional'])

        # Build structured slide input — full content, not truncated
        all_topics = []
        # Cover: overall post title sets channel/pillar context
        all_topics.append(f"POST TITLE (channel context): {title}")
        for slide in slides:
            raw_title = (slide.get('title', '') or '').strip()
            full_body = (slide.get('content_en', '') or slide.get('content', '') or slide.get('content_es', '')).strip()
            if raw_title and full_body:
                t = f"SLIDE TITLE: {raw_title}\nSLIDE BODY: {full_body}"
            elif raw_title:
                t = f"SLIDE TITLE: {raw_title}"
            else:
                t = f"SLIDE BODY: {full_body}"
            all_topics.append(t)
        
        try:
            scene_prompt = (
                f"You are generating background images for a LinkedIn carousel post.\n"
                f"POST TOPIC (channel/pillar context): {title}\n"
                f"VISUAL STYLE: {style} — {visual_tone}\n"
                f"COLOR PALETTE: {color_tone}\n\n"
                "For each numbered slide below, follow this TWO-STEP reasoning process:\n"
                "  STEP 1 — Read the slide title and body. Identify the SPECIFIC action, result, or claim the slide is making "
                "(not just the industry — the actual thing happening: a partnership deployed, a metric achieved, a technology installed, a problem solved).\n"
                "  STEP 2 — Describe a photographic scene (15-20 words max) that makes that specific claim VISIBLE to someone "
                "who cannot read any text. The scene must feel like a still frame from a documentary shot at the exact moment described.\n\n"
                "ABSOLUTE RULES:\n"
                "- Each scene MUST be visually distinct — different subject, physical setting, and camera angle.\n"
                "- NEVER use generic offices, laptops on desks, or abstract light bokeh unless the slide is specifically about desk work.\n"
                "- Describe ONLY physical objects, people, settings, lighting, and textures. NO screens, NO monitors, NO text of any kind.\n"
                "- The scene MUST reflect what THIS specific slide claims - not just the general topic of the post.\n"
                "- NO futuristic concept vehicles, NO sci-fi machinery, NO impossible technology; use realistic currently deployable industrial settings.\n"
                "- Output ONLY the numbered scene descriptions, one per line. No step labels, no explanations, no extra text.\n\n"
                + "\n\n".join(f"{i+1}.\n{t}" for i, t in enumerate(all_topics))
            )
        ...
        accent_rgb = self._get_accent_color(style)
        image_prompts['cover'] = (
            f"Photorealistic, {style_desc} aesthetic, high-quality photograph: {cover_scene}. "
            f"Color mood: {color_tone}. Match palette accents around RGB {accent_rgb}. "
            f"16:9 aspect ratio. {no_text_instruction}"
        )
        
        # Build slide prompts
        variation_lenses = [
            "wide composition, environmental context",
            "mid-shot with human activity and hands-on action",
            "close-up texture and details",
            "dynamic perspective with depth and motion cues",
            "structured geometric composition",
            "natural lighting with high contrast silhouettes",
            "top-down operational arrangement"
        ]
        for idx, slide in enumerate(slides):
            slide_title = (slide.get('title', '') or '').strip()
            slide_body = (slide.get('content_en', '') or slide.get('content', '') or slide.get('content_es', '')).strip()
            slide_scene = scene_descriptions.get(idx + 2, "a realistic operational industrial environment showing measurable work in progress")
            intent_hint = self._derive_intent_visual_hint(slide_title, slide_body)
            visual_mode = self._choose_visual_mode(slide_title, slide_body)

            # Enforce meaning alignment: if scene is generic, regenerate with
            # slide-specific context until we reach a minimum relevance score.
            score, reasons = self._scene_relevance_score(slide_scene, slide_title, slide_body)
            if score < 0.45:
                print(f"DEBUG: Slide {idx+1} scene low relevance ({score:.2f}) -> {reasons}; regenerating")
                for regen_attempt in range(2):
                    try:
                        improved = self._generate_scene_for_single_slide(
                            post_title=title,
                            style=style,
                            ...
                            previous_scene=slide_scene,
                        )
                        new_score, new_reasons = self._scene_relevance_score(improved, slide_title, slide_body)
                        print(f"DEBUG: Slide {idx+1} regenerated scene attempt {regen_attempt+1}: score={new_score:.2f}")
                        if new_score > score:
                            slide_scene = improved
                            score = new_score
                            reasons = new_reasons
                        if score >= 0.45:
                            break
                    except Exception as regen_err:
                        print(f"DEBUG: Slide {idx+1} scene regeneration failed: {regen_err}")

            lens = variation_lenses[idx % len(variation_lenses)]
            if visual_mode == 'symbolic':
                image_prompts[f'slide_{idx}'] = (
                    f"Minimal, premium conceptual illustration with one central symbol representing this idea: {intent_hint}. "
                    f"Support scene context: {slide_scene}. "
                    "Use a single clear icon-like cue (for example check mark, upward arrow, shield, or bridge) integrated into a realistic industrial setting. "
                    "Keep composition very simple, uncluttered, and high-contrast. "
                    f"Composition: {lens}. Color mood: {color_tone}. Match palette accents around RGB {accent_rgb}. "
                    "No brand logos. No fictional sci-fi elements. "
                    f"{no_text_instruction}"
                )
            elif visual_mode == 'infographic':
                image_prompts[f'slide_{idx}'] = (
                    f"Clean, high-end isometric industrial infographic illustration: {slide_scene}. "
                    f"Visual language: operational flow, equipment blocks, process arrows, data-to-action metaphor, no UI screens. "
                    f"Intent focus: {intent_hint}. Keep visual simple with one primary operational idea. "
                    f"Composition: {lens}. Color mood: {color_tone}. Match palette accents around RGB {accent_rgb}. "
                    "No brand logos. No fictional sci-fi elements. "
                    f"{no_text_instruction}"
                )
                ...
                image_prompts[f'slide_{idx}'] = (
                    f"Photorealistic, {style_desc} aesthetic, high-quality photograph: {slide_scene}. "
                    f"Intent focus: {intent_hint}. Keep composition simple, documentary style, one clear operational action, max four people. "
                    f"Composition: {lens}. Color mood: {color_tone}. Match palette accents around RGB {accent_rgb}. "
                    "No brand logos. No fictional sci-fi elements. "
                    f"{no_text_instruction}"
                )
        ...
        # Pre-compute display titles first so translations match what is actually rendered.
        normalized_slide_titles = []
        for slide in slides:
            candidate_title = (slide.get('title', '') or '').strip()
            content_en = slide.get('content_en', '')
            content_fallback = slide.get('content', '')

            if not candidate_title or candidate_title.startswith('Key Point'):
                content_preview = content_en or content_fallback
                candidate_title = self._create_summary_title(content_preview, max_chars=65)
            elif len(candidate_title) > 65:
                candidate_title = self._create_summary_title(candidate_title, max_chars=65)

            normalized_slide_titles.append(candidate_title)
        ...
        if is_bilingual_carousel:
            ...
            for idx, t in enumerate(normalized_slide_titles):
                if t:
            ...
            try:
                batch_prompt = (
                    "You are translating LinkedIn carousel slide titles for a professional post about mining, technology, or business.\n"
                    "RULES:\n"
                    "1. Write NATURAL, idiomatic Spanish — do NOT translate word-for-word. Restructure phrases so they sound like a native Spanish speaker wrote them.\n"
                    "2. KEEP company names, brand names, product names, and proper nouns EXACTLY as they appear in English (e.g. 'Applied Intuition', 'Heidelberg Materials', 'AI', 'SAP', 'Komatsu' stay unchanged).\n"
                    "3. If a title is a short fragment or incomplete phrase, complete it naturally in Spanish so it reads as a coherent title.\n"
                    "4. Prefer active, concise phrasing — avoid long literal translations.\n"
                    "5. Return ONLY the numbered lines in the EXACT same numbered format. No explanations, no extra text.\n\n"
                    "Translate each numbered line below:\n"
                ...
                for idx, t in enumerate(normalized_slide_titles):
                    if t and idx not in slide_titles_es:
                        try:
                            individual = self._call_gemini(
                                f"Translate this LinkedIn carousel slide title from English to natural Spanish.\nRULES: Keep company/brand names in English. Write idiomatic Spanish, not word-for-word. Return ONLY the Spanish title, nothing else.\nTitle: {t}"
                            )
                            cleaned = individual.strip().strip('"\'\'\u201c\u201d')
                            if self._looks_untranslated(t, cleaned):
                                retry = self._call_gemini(
                                    f"Rewrite this as a natural Spanish LinkedIn carousel title. Keep proper nouns in English. Return only the Spanish title: {t}"
                                )
                                cleaned = retry.strip().strip('"\'\'\u201c\u201d')
                            slide_titles_es[idx] = cleaned if cleaned else t
                ...
                for idx, t in enumerate(normalized_slide_titles):
                    if t:
                        try:
                            individual = self._call_gemini(f"Translate this LinkedIn carousel slide title from English to natural Spanish.\nRULES: Keep company/brand names in English. Write idiomatic Spanish, not word-for-word. Return ONLY the Spanish title, nothing else.\nTitle: {t}")
                            cleaned = individual.strip().strip('"\'\'\u201c\u201d')
                            if self._looks_untranslated(t, cleaned):
                                retry = self._call_gemini(
                                    f"Rewrite this as a natural Spanish LinkedIn carousel title. Keep proper nouns in English. Return only the Spanish title: {t}"
        ...
        try:
            ...
            if not cover_img:
                ...
                cover_img = self._harmonize_image_with_style(cover_img, style)
        ...
        for idx, slide in enumerate(slides):
            ...
            slide_title = normalized_slide_titles[idx] if idx < len(normalized_slide_titles) else (slide.get('title', '') or '')
            content_en = slide.get('content_en', '')
            ...
            try:
                ...
                if not slide_img:
                    ...
                    slide_img = self._harmonize_image_with_style(slide_img, style)
    ...
    def _translate_to_spanish(self, english_text: str) -> str:
        ...
        try:
            prompt_text = f"""You are translating a LinkedIn carousel slide title for a professional post about mining, technology, or business.
RULES:
1. Write NATURAL, idiomatic Spanish — do NOT translate word-for-word. Restructure so it sounds like a native speaker wrote it.
2. Keep company names, brand names, product names, and proper nouns EXACTLY as they appear in English.
3. Return ONLY the Spanish translation — no explanations, no alternatives, no extra text.
...
Spanish:"""
    ...
    def _looks_untranslated(self, english_text: str, spanish_text: str) -> bool:
        """Detect obvious untranslated output so we can retry translation."""
        import re
        a = re.sub(r'[^a-z0-9]+', ' ', (english_text or '').lower()).strip()
        b = re.sub(r'[^a-z0-9]+', ' ', (spanish_text or '').lower()).strip()
        return bool(a) and a == b

    def _extract_slide_keywords(self, title: str, body: str, max_terms: int = 8) -> List[str]:
        """Extract compact keyword set to check scene-vs-slide semantic alignment."""
        import re
        text = f"{title or ''} {body or ''}".lower()
        tokens = re.findall(r'[a-z][a-z0-9\-]{2,}', text)
        stop = {
            'the', 'and', 'for', 'with', 'that', 'this', 'from', 'into', 'your', 'our', 'you',
            'are', 'was', 'were', 'been', 'will', 'have', 'has', 'had', 'about', 'not', 'just',
            'para', 'como', 'con', 'una', 'por', 'del', 'las', 'los', 'que', 'esta', 'este',
            'more', 'than', 'their', 'they', 'them', 'its', 'it', 'but', 'can', 'across', 'over'
        }
        # Keep order, drop duplicates, and favor meaningful terms.
        result = []
        for t in tokens:
    ...
    def _scene_relevance_score(self, scene: str, slide_title: str, slide_body: str) -> Tuple[float, str]:
        """Score whether generated scene text matches slide meaning and avoids generic visuals."""
        import re
        s = (scene or '').lower()
        if not s:
            return 0.0, 'empty scene description'

        keywords = self._extract_slide_keywords(slide_title, slide_body)
        if not keywords:
            return 0.5, 'no keywords extracted; neutral score'

        hits = 0
        for kw in keywords:
            stem = re.escape(kw[:6])
            if re.search(rf'\b{stem}[a-z0-9\-]*\b', s):
                hits += 1

        base = hits / max(1, len(keywords))

        generic_terms = [
            'office', 'desk', 'laptop', 'computer', 'boardroom', 'meeting room',
            'futuristic', 'sci-fi', 'neon', 'bokeh', 'abstract background'
        ]
        context = f"{slide_title or ''} {slide_body or ''}".lower()
        office_allowed = any(k in context for k in ['office', 'executive', 'management meeting', 'corporate'])

        penalty = 0.0
        generic_hits = []
        for t in generic_terms:
            if t in s:
                if office_allowed and t in {'office', 'boardroom', 'meeting room'}:
                    continue
                generic_hits.append(t)
                penalty += 0.09

        score = max(0.0, min(1.0, base - penalty))
        reason = f"keyword_hits={hits}/{len(keywords)}"
        if generic_hits:
            reason += f", generic={','.join(generic_hits[:3])}"
        return score, reason

    def _is_infographic_candidate(self, slide_title: str, slide_body: str) -> bool:
        """Route conceptual/process-heavy slides to non-photographic infographic imagery."""
        text = f"{slide_title or ''} {slide_body or ''}".lower()
        triggers = {
            'framework', 'model', 'roadmap', 'steps', 'process', 'system architecture',
            'maturity model', 'methodology', 'matrix', 'decision tree', 'workflow diagram'
        }
        return any(t in text for t in triggers)

    def _derive_intent_visual_hint(self, slide_title: str, slide_body: str) -> str:
        """Derive a concise visual-action hint from slide intent to avoid generic scenes."""
        text = f"{slide_title or ''} {slide_body or ''}".lower()

        if any(k in text for k in ['identify', 'opportunit', 'diagnos', 'assessment', 'evaluate']):
            return 'operations team identifying improvement opportunities in the field using physical workflow cues and equipment context'
        if any(k in text for k in ['utilization', 'productivity', 'output', 'throughput', 'efficiency']):
            return 'visible productivity improvement in active operations: coordinated crew activity, equipment in motion, and measurable workflow progression'
        if any(k in text for k in ['autonomy', 'autonomous', 'readiness', 'deployment']):
            return 'practical autonomy readiness scene with current industrial equipment, safety observers, and deployment preparation tasks'
        if any(k in text for k in ['reduce', 'variability', 'stability', 'consistency']):
            return 'standardized operational process with clear repetition and control, emphasizing reduced variability and stable execution'
        if any(k in text for k in ['bridge', 'gap', 'current state', 'future state', 'transition']):
            return 'transition moment showing current operation and improved target workflow side by side in one realistic setting'

        return 'simple realistic operational scene directly tied to the slide claim, with one main action and clear industrial context'

    def _choose_visual_mode(self, slide_title: str, slide_body: str) -> str:
        """Choose visual style mode per slide: photo, infographic, or simple symbolic."""
        text = f"{slide_title or ''} {slide_body or ''}".lower()

        symbolic_triggers = {
            'check', 'checkmark', 'verify', 'validated', 'approved', 'ready', 'done',
            'priority', 'opportunity', 'win', 'success', 'compliance', 'safe', 'safety'
        }
        if any(t in text for t in symbolic_triggers):
            return 'symbolic'

        if self._is_infographic_candidate(slide_title, slide_body):
            return 'infographic'

        return 'photo'

    def _generate_scene_for_single_slide(
        self,
        post_title: str,
        style: str,
        visual_tone: str,
        ...
        """Regenerate one slide scene using full context when batch output is too generic."""
        prompt = (
            "Generate ONE photographic scene description for a LinkedIn carousel background.\n"
            f"POST CONTEXT: {post_title}\n"
            f"STYLE: {style} ({visual_tone})\n"
            f"COLOR MOOD: {color_tone}\n"
            f"SLIDE TITLE: {slide_title}\n"
            f"SLIDE BODY: {slide_body}\n"
            f"PREVIOUS GENERIC/WRONG SCENE TO AVOID: {previous_scene}\n\n"
            "Rules:\n"
            "- 15-24 words only.\n"
            "- Must visualize the specific claim/action/result from this slide.\n"
            "- No generic office/laptop scene unless the slide is explicitly about office work.\n"
            "- No futuristic concept vehicles or sci-fi elements.\n"
            "- Describe concrete people, machinery, tools, setting, and physical action.\n"
            "- No text, signs, labels, screens, or UI references.\n"
            "Return only the scene line, no numbering, no explanation."
        )
        result = self._call_gemini(prompt).strip().replace('**', '').replace('*', '')
        if '\n' in result:
            result = result.split('\n', 1)[0].strip()
        return result

    def _harmonize_image_with_style(self, img: Image.Image, style: str) -> Image.Image:
        """Apply a subtle style tint so AI images blend with carousel palette."""
        try:
            rgb = self._get_accent_color(style)
            base = img.convert('RGB')
            tint_layer = Image.new('RGB', base.size, rgb)
            blended = Image.blend(base, tint_layer, 0.16)
